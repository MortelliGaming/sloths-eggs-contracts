...
pub contract SlothsEggsBud: FungibleToken {

    /// Total supply of SlothsEggsBuds in existence
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance {
        ...
        pub fun deposit(from: @FungibleToken.Vault) {
            let vault <- from as! @SlothsEggsBud.Vault
        ...
        destroy() {
            if(self.balance > 0.0) {
              ...
              SlothsEggsBud.totalSupply = SlothsEggsBud.totalSupply.saturatingSubtract(self.balance)
            }
        }
    }
    ...
    access(account) fun mintTokensForPlayer(amount: UFix64, playerAddress: Address) {
        ...
        var oldAmount = SlothsEggsBud.totalSupply
        SlothsEggsBud.totalSupply = SlothsEggsBud.totalSupply.saturatingAdd(amount)
        emit TokensMinted(amount: SlothsEggsBud.totalSupply - oldAmount)
        var newVault <- create Vault(balance: SlothsEggsBud.totalSupply - oldAmount)
    ...
    pub resource Administrator {
        ...
        ///
        pub fun mintTokens(amount: UFix64): @SlothsEggsBud.Vault {
            pre {
                amount > 0.0: "Amount minted must be greater than zero"
            }
            var oldAmount = SlothsEggsBud.totalSupply
            SlothsEggsBud.totalSupply = SlothsEggsBud.totalSupply.saturatingAdd(amount)
            emit TokensMinted(amount: SlothsEggsBud.totalSupply - oldAmount)
            return <-create Vault(balance: SlothsEggsBud.totalSupply - oldAmount)
        }
    }
    ...
    init() {
        ...
        //
        destroy self.account.load<@AnyResource>(from: /storage/slothMiningWarBudVault)
        let vault <- create Vault(balance: self.totalSupply)
        self.account.save(<-vault, to: /storage/slothMiningWarBudVault)

        // Create a public capability to the stored Vault that only exposes
        // the `deposit` method through the `Receiver` interface
        //
        self.account.link<&{FungibleToken.Receiver}>(
            /public/slothMiningWarBudReceiver,
            target: /storage/slothMiningWarBudVault
        )

        // Create a public capability to the stored Vault that only exposes
        // the `balance` field through the `Balance` interface
        //
        self.account.link<&SlothsEggsBud.Vault{FungibleToken.Balance}>(
            /public/slothMiningWarBudBalance,
            target: /storage/slothMiningWarBudVault
        )
        destroy self.account.load<@AnyResource>(from: /storage/slothsEggsBudAdmin)
        let admin <- create Administrator()
        self.account.save(<-admin, to: /storage/slothsEggsBudAdmin)
